...
fs.copyFileSync(
  path.join(SRC_DIR, "search.js"),
  path.join(DIST_DIR, "search.js"),
);
fs.copyFileSync(
  path.join(SRC_DIR, "ai-assistant.js"),
  path.join(DIST_DIR, "ai-assistant.js"),
);
fs.copyFileSync(
  path.join(SRC_DIR, "mobile-menu.js"),
  path.join(DIST_DIR, "mobile-menu.js"),
);
fs.copyFileSync(
  path.join(SRC_DIR, "demos.js"),
  path.join(DIST_DIR, "demos.js"),
);

// Copy support page
const supportPath = path.join(__dirname, "../support.html");
if (fs.existsSync(supportPath)) {
  fs.copyFileSync(supportPath, path.join(DIST_DIR, "support.html"));
  console.log("Copied support.html to dist/");
}
...
if (!fs.existsSync(path.join(DIST_DIR, "index.html"))) {
  ...
  if (!pages.find((p) => p.slug === "index.html") && pages.length > 0) {
    fs.copyFileSync(
      path.join(DIST_DIR, pages[0].slug),
